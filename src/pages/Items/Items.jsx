...
export default function Items() {
  ...
  useEffect(() => {
    async function loadItems() {
      ...
      try {
        ...
        setAllItems(hybridItems);
      } catch (error) {
        console.error('Failed to load hybrid items:', error);
        setAllItems([]);
      } finally {
        setLoading(false);
      }
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.minPrice !== '') {
      result = result.filter((i) => Number(i.price) >= Number(filters.minPrice));
    }

    if (filters.maxPrice !== '') {
      result = result.filter((i) => Number(i.price) <= Number(filters.maxPrice));
    }
      ...
      <div className="container">
        ...
        <div className="items-page__toolbar">
          ...
          <div className="items-page__controls">
            <button
              className="items-page__filter-toggle"
            ...
            <div className="items-page__view-toggle">
              <button
                className={`items-page__view-btn${
                  view === 'grid' ? ' items-page__view-btn--active' : ''
                }`}
              ...
              <button
                className={`items-page__view-btn${
                  view === 'list' ? ' items-page__view-btn--active' : ''
                }`}
            ...
            {loading ? (
              ...
              <div className="items-page__empty">
                ...
                <button
                  ...
                  className="items-page__reset-btn"
                >
                  Clear All Filters
                </button>
              </div>
              ...
              <div
                className={`items-page__grid${
                  view === 'list' ? ' items-page__grid--list' : ''
                }`}
              >
                {paginated.map((item) => (
